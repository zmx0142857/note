...
- **安装win10** 【新建虚拟机/自定义安装】
  - 选择 iso 文件
  - 名称: `win10-yymmdd`
  - 安装位置: `D:\app\vm\win10-yymmdd`
  - CPU: 1个，8核, 后续可以修改
  - 内存: 8GB, 后续可以修改
  - 硬盘: 100GB. 后续可以再加硬盘.
    虚拟机通常不会真的占用 100GB 的物理机空间:
    实测全新安装的 win10 专业版, 占用的物理机空间只有 12.3GB.
- **文件传输** 安装 VM Ware tools 后, 即可通过拖拽或剪贴板的方式在宿主与虚拟机之间传输文件.
- **快照备份** 先关闭虚拟机，然后【虚拟机/快照/拍摄快照】
- **克隆** 先关闭虚拟机，然后【虚拟机/管理/克隆】

## Virtual Box

简称 vbox. oracle 公司的虚拟机软件（免费）
...
- 处理器: 8核
- 不勾选 Enable EFI
- Disk Size: 80GB. 和 VM Ware 不同, 需要确保物理机真的有 80GB 空间
