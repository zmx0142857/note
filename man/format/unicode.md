...
### 变体选择符 (Variation Selector, VS)

范围 `U+FE00 ~ U+FEFF`, 其中 `U+FE0F` 可以把字符变为 emoji.
...
参见: [空格字符一览](https://cloud.tencent.com/developer/article/2128593)
...
|U+202E|强制右至左|RIGHT-TO-LEFT-OVERRIDE|rlo||

### QQ 昵称小技巧

> 2025 更新: `\u202e` 已被禁用, 可以改用 `昵称\u202d\u202d\u2067~喵\u2067\u202d`

用 `昵称\u202e~喵\u202d` 做 QQ 昵称, 群友 @ 你时就会在句末带 `喵~`. 原理如下:
```txt
输出 "@昵称\u202e~喵\u202d 群友对你说的话", 用 "|" 表示光标. 遇到控制字符时光标转向:
...
## 中日韩兼容表意文字补充区 (CJK Compatibility Ideographs Supplement)

范围 U+F800 ~ U+FAFF. 这是 unicode 的屎山, 已经有的文字要再造一个, 用来兼容历史文件!
```js
'兀'.codePointAt(0).toString(16) // 5140, 正常
'兀'.codePointAt(0).toString(16) // fa0c, 兼容区
```
