...
class TransportProblem {
    ...
    *nonBase() {
    ...
    transport(row, col) {
        ...
        if (this.isZero(produce[row]))
            rowVisited[row] = true;
        if (this.isZero(consume[col]))
            colVisited[col] = true;
    }
    // 计算一行/列的最小元素与次小元素的差
    getDiff(arr) {
    ...
    // 求解运输问题
    solve({ initBy = 'min', checkBy = 'potential' } = {}) {
        switch (initBy) {
            case 'min':
                this.initSolutionByMin();
                break;
            case 'vogel':
                this.initSolutionByVogel();
                break;
