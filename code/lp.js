...
class LP {
    static makeArr(length, callback) {
    ...
    solveInfo({ log = false } = {}) {
        ...
        if (res === LP.status.FOUND) {
            ...
            const varValues = [];
            let posValue;
            const getValue = (index) => index === -1 ? 0 : this.mat[index][m + n];
            this.originVars.forEach((v, i) => {
                ...
                if (v.endsWith('+')) { // 无约束变量配对使用
                    ...
                    varValues.push(v + ': ' + getValue(index)); // 非负变量直接使用
            });
     ...
     * @param lineNo 行号
     */
    static parseExpr(input, vars, freeVars, row, sign = 1, lineNo = 0) {
    ...
    static addCoef(coef, sym, vars, freeVars, row) {
